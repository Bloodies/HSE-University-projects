...
namespace Lab._3__Threads_war_
{
    class Program
    {
        public class Text
        ...
        static void Main(string[] args)
        {
            ...
            Text.score();
            scripts.Gun gun = new scripts.Gun();
            new Thread(scripts.Enemy.CreateEnemy).Start();
            while (true)
            {
                ...
                switch (key.Key)
                {
                    case ConsoleKey.Spacebar:
                        new Thread(scripts.Bullet.CreateBullet).Start(gun.X);
                        Thread.Sleep(100);
                        break;
